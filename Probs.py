...
def sim_mdp(dlr,cards, p, depth=5):
    ...
    if total==21:
        ...
        hit=non_face * sum([sim_mdp(dlr,cards+[i+1],p)[0] for i in range(9)]) + p*sim_mdp(dlr,cards+[10],p)[0]
        #check if doubling is allowed
        if len(cards)==2:
            double=2* non_face * sum([calc_stand(cards+[i+1],dlr) for i in range(9)]) + 2*p*calc_stand(cards+[10],dlr)
        ...
        if len(cards)==2 and cards[0]==cards[1] and depth>0: 
            ...
            if cards[0]==1:
                split=non_face * sum([calc_stand([1,i+1],dlr) for i in range(9)]) + p*calc_stand([1,10],dlr)
                split=2*split-1
            else:
                split=non_face * sum([sim_mdp(dlr,[cards[0],i+1],p,depth-1)[0] for i in range(9)]) + p*sim_mdp(dlr,[cards[0],10],p,depth-1)[0]
                split=2*split-1
        else:
            split=-1
